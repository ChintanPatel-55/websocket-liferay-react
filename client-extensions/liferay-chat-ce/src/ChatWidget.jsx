...
const ChatWidget = () => {
        ...
        socketRef.current.onmessage = (event) => {
            ...
            try {
                ...
                if (json.type === "ONLINE_USERS") {
                    const rawList = json.activeIds || json.activeUserIds || [];
                    const ids = rawList.map(id => String(id));
    ...
    const parseMessage = (msg) => {
        ...
        if (typeof msg !== 'string') return { isMine: false, sender: 'System', content: '', isSystem: true };
    ...
    const isUserOnline = (uid) => {
        return onlineUserIds.includes(String(uid));
    };
        ...
        <div style={styles.mainContainer}>
            {showSidebar && (
                <div style={styles.sidebar}>
                    <div style={styles.sidebarHeader}>
                        ...
                        <div style={styles.countBadge}>{filteredUserList.length}</div>
                    </div>
                                ...
                                >
                                    ...
                                    <div style={{display:'flex', flexDirection:'column', overflow:'hidden'}}>
                                        ...
                                        <div style={{display:'flex', alignItems:'center', gap:'5px'}}>
                                            ...
                                            <span style={{fontSize:'11px', color: online ? '#4caf50' : '#888'}}>
            ...
            <div style={styles.chatContainer}>
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:'8px'}}>
                        ...
                        <div style={{display:'flex', flexDirection:'column'}}>
                            <span style={{fontSize:'14px', fontWeight:'bold'}}>
                                {targetUserName ? targetUserName : "Select a User"}
                            </span>
                            {targetUserId && (
                                <div style={{display:'flex', alignItems:'center', gap:'4px'}}>
                                    <div style={{
                                        width: '6px', height: '6px', borderRadius: '50%',
                                        background: isUserOnline(targetUserId) ? '#aaffaa' : '#ccc'
                                    }}></div>
                                    <span style={{fontSize:'10px', color: '#fff', opacity: 0.9}}>
                                        {isUserOnline(targetUserId) ? "Online" : "Offline"}
                                    </span>
                                </div>
                            )}
                        </div>
                ...
                <div style={styles.controls}>
                    <input type="text" placeholder={targetUserId ? "Type a message..." : "Select a user..."}
                           value={inputText} disabled={!targetUserId} onChange={handleInputChange} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} style={styles.inputMsg} />
...
const styles = {
    ...
    countBadge: { fontSize:'10px', background:'#4caf50', color:'#fff', padding:'2px 6px', borderRadius:'10px' },
